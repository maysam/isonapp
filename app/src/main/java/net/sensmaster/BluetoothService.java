...
public class BluetoothService {
  ...
  private static final int       STATE_NONE            = 0;                 // we're
                                                                             // doing
                                                                             // nothing
  private static final int       STATE_CONNECTING      = 2;                 // now
                                                                             // initiating
                                                                             // an
                                                                             // outgoing
                                                                             // connection
   ...
   */
  public BluetoothService(MessageHandler handler) {
  ...
  private synchronized void setState(int state) {
    ...
    mHandler.obtainMessage(Monitor.MESSAGE_STATE_CHANGE, state, -1)
        .sendToTarget();
  }
  ...
  public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
    ...
    SharedPreferences settings = mHandler.getSharedPreferences();
  ...
  public void write(byte[] out) {
    ...
    r.write(out);
    Log.v("write", String.format("%x", new BigInteger(1, out)));
  }

  private void sendBytesWritten(byte[] bytes) {
    Log.v("written", Arrays.toString(bytes));
  ...
  private void connectionFailed() {
    ...
    Bundle bundle = new Bundle();
    bundle.putString(Monitor.TOAST, "Unable to connect device");
    msg.setData(bundle);
    mHandler.sendMessage(msg);
  ...
  private void connectionLost() {
    ...
    Message msg = mHandler.obtainMessage(Monitor.MESSAGE_TOAST);
    Bundle bundle = new Bundle();
    bundle.putString(Monitor.TOAST, "Device connection was lost");
    msg.setData(bundle);
    mHandler.sendMessage(msg);
  ...
  private class ConnectThread extends Thread {
    ...
    public ConnectThread(BluetoothDevice device) throws SecurityException,
      ...
      Log.v(TAG, "calling device.createRfcommSocket with channel 1 ...");
      try {
        ...
        Log.v(TAG, "setting socket to result of createRfcommSocket");
      } catch (Exception e) {
        Log.e(TAG, e.getMessage(), e);
      }
      mmSocket = tmp;
  ...
  private class ConnectedThread extends Thread {
    ...
    private volatile boolean hasReadAnything = false;
    ...
    public void run() {
      ...
      Data data = new Data(mHandler);
      while (!stop) {
        try {
          byte[] buffer = new byte[1024];
          int result = mmInStream.read(buffer);
          if (result > 0) {
            hasReadAnything = true;
            byte[] sized_buffer = Arrays.copyOf(buffer, result);
            //          Log.i(TAG, String.format("read %d: %x", result, new BigInteger(1, sized_buffer)));
            data.write(sized_buffer);
          }
        } catch (IOException e) {
